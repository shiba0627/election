...
class GUIApp():
    def __init__(self):
        ...
        #ボタンの設定(通常, 待機, 待機選択, 選択, 中心X割合, 中心Y割合, 横長さ, 縦長さ, コマンド, 段階)
        button_list = [
            (CANDIDATES_1, CANDIDATES_1_WAIT, CANDIDATES_1,CANDIDATES_1_WAIT,  6/100,5/10,CANDIDATES_WHIDTH, CANDIDATES_HEIGHT,'1',1),
            (CANDIDATES_1, CANDIDATES_1_WAIT, CANDIDATES_1,CANDIDATES_1_WAIT, 17/100,5/10,CANDIDATES_WHIDTH, CANDIDATES_HEIGHT,'2',1),
            (CANDIDATES_1, CANDIDATES_1_WAIT, CANDIDATES_1,CANDIDATES_1_WAIT, 28/100,5/10,CANDIDATES_WHIDTH, CANDIDATES_HEIGHT,'3',1),
            (CANDIDATES_1, CANDIDATES_1_WAIT, CANDIDATES_1,CANDIDATES_1_WAIT, 39/100,5/10,CANDIDATES_WHIDTH, CANDIDATES_HEIGHT,'4',1),
            (CANDIDATES_1, CANDIDATES_1_WAIT, CANDIDATES_1,CANDIDATES_1_WAIT, 50/100,5/10,CANDIDATES_WHIDTH, CANDIDATES_HEIGHT,'5',1),
            (YES, YES_WAIT    , YES, YES_WAIT, 75/100, 3/8, BUTTON_SIZE, BUTTON_SIZE,'a',2),
        ...
        self.buttons = []
        for img_path, clicked_path, lock_path, attention_path, center_x_ratio, center_y_ratio, w, h, cmd, step in button_list:
            center_x = center_x_ratio * self.width
            center_y = center_y_ratio * self.height
            area = self.calc_area(center_x, center_y, w, h)
            self.buttons.append(makeButton(self.canvas, img_path, clicked_path, lock_path, attention_path, area, cmd, step))
